...
from src.monitoring import DriftReport, DriftSeverity, detect_drift, population_stability_index
...
def test_detect_drift_with_dataframes():
    ...
    assert drift_report.features_checked == 3
    assert len(drift_report.feature_results) == 3
...
def test_detect_drift_significant_shift():
    ...
    # Drift deve ser detectado (verificar severidade não-nula)
    assert isinstance(drift_report, DriftReport)
    assert drift_report.features_with_drift >= 1
    assert drift_report.overall_severity in [DriftSeverity.MODERATE, DriftSeverity.HIGH]
